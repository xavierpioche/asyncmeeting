...
//----------------------
			...
			if ($is_mod) {
				$a_text=$_POST['a_text'];
				agendum_mod_name($a_id,$a_text);
				header('Location: meeting_show.php?m_id='.$m_id.'&a_id='.$a_id);
			} else {
				$old_text=agendum_get_name($a_id);
				echo "<form name=form1 action=agendum_edit.php?is_mod=1&a_id=".$a_id."&m_id=".$m_id." method=post>";
				echo "<input size=50 type=text name=a_text value='".$old_text."'><br>";
				echo "<input type=submit value='".get_label(35,$language)."'><br><br><br>";
				echo "<a href=meeting_show.php?m_id=".$m_id."&a_id=".$a_id.">".get_label(15,$language)."</a>";
			}
